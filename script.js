...
const startIntroSequence = () => {
    ...
    if (introVideo) {
        ...
        const selectedSource = isMobileView
            ? introVideo.dataset.mobileSrc
            : introVideo.dataset.desktopSrc;
        const sourceElement = introVideo.querySelector("source");
        const revealVideo = () => {
        ...
        if (sourceElement && selectedSource) {
            sourceElement.setAttribute("src", selectedSource);
            introVideo.load();
        }
        ...
        const syncFallbackToVideo = () => {
            window.clearTimeout(fallbackTimer);

            const durationMs = Number.isFinite(introVideo.duration) && introVideo.duration > 0
                ? Math.ceil(introVideo.duration * 1000) + 300
                : fallbackDuration;

            fallbackTimer = window.setTimeout(finishIntro, durationMs);
        };

        if (introVideo.readyState >= 1) {
            syncFallbackToVideo();
        } else {
            introVideo.addEventListener("loadedmetadata", syncFallbackToVideo, { once: true });
